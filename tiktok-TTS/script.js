...
window.onload = () => {
  ...
  if (resp.data) {
    if (resp.data.available) {
      console.info(
        `${resp.data.meta.dc} (age ${resp.data.meta.age} minutes) is able to provide service`
      );
      enableControls();
    } else {
      console.error(
        `${resp.data.meta.dc} (age ${resp.data.meta.age} minutes) is unable to provide service`
      );
      setError(
        `Service not available${
          resp.data.message && resp.data.message.length > 1
            ? ` (<b>"${resp.data.message}"</b>)`
            : ""
        }, try again later`
      );
    }
  } else {
    setError("Error querying API status, try again later");
  }
};
...
const submitForm = () => {
  ...
  if (textLength > TEXT_BYTE_LIMIT) {
    setError(
      `Text must not be over ${TEXT_BYTE_LIMIT} UTF-8 chracters (currently at ${textLength})`
    );
    enableControls();
    return;
  }
  ...
  try {
    ...
    console.log(
      "^ Please take a screenshot of this and create an issue on the GitHub repository if one does not already exist :)"
    );
    console.log(
      "If the error code is 503, the service is currently unavailable. Please try again later."
    );
